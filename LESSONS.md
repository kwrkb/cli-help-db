...
## コードレビュー対応 (2026-03-27)

### [Gotcha] Go Fprintf テンプレート内の `%%` を bash の `%%` と混同しない
- **状況**: 外部レビューで「`${TRY_KEY%%__*}` が bash の最長一致になっている」と指摘されたが、これは Go の `fmt.Fprintf` テンプレート内の `%%`（= エスケープ）であり、出力される bash は `${TRY_KEY%__*}`（最短一致）で正しい
- **ルール**: hook テンプレートのレビュー時は、Go ソース上の `%%` と bash 出力の `%` を区別する。テンプレートの動作確認は生成後のスクリプトを見て判断する

### [Pattern] config エラーハンドリングはフラグ明示/暗黙で分岐する
- **状況**: `list` と `hook` が config ロードエラーを全てサイレントにフォールバックしていた。malformed YAML やパーミッションエラーも握り潰されるため、誤設定の検出が困難だった
- **ルール**: `--config` 明示時は fail fast（`build` と同じ）、デフォルト探索失敗時のみ `DefaultConfig()` にフォールバック。新サブコマンド追加時もこのパターンに従う
